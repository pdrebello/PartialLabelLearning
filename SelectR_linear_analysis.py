...
import numpy as np
import scipy.io
from dataset import Dataset, loadTrain, loadTrainT, loadTrainValHelp
import sys
...
def q_test_old(test_data, q_net, rl_technique):
    q_net.eval()
    test_loss = 0
    correct = 0
    with torch.no_grad():
    ...
    with torch.no_grad():
        for data, target in test_data:
            ...
            for index, prediction in enumerate(pred):
                if(target[index][prediction] == 1):
                    correct+=1
                confidence_sum += output[index][prediction]
            #targ_pred = target.data.max(1, keepdim=True)[1]
            #correct += pred.eq(targ_pred.data.view_as(pred)).sum()
      
    return (100. * float(correct) / len(test_data.dataset)), confidence_sum/len(test_data.dataset)
...
k = 10
dump_dir = "results/05022021"
datasets = ['lost','MSRCv2','BirdSong','Yahoo! News','Soccer Player']
datasets = ['lost','MSRCv2','BirdSong','Soccer Player']
#datasets = ['Yahoo! News']
                ...
                for fold_no in range(k):
                    if(filename == 'Soccer Player'):
                        if(tech == 'select'):
                            if(fold_no == 0):
                                fold_no = 1
                    train_dataset, test_dataset, real_train_dataset, val_dataset, input_dim, output_dim = loadTrainValHelp(filename+".mat", fold_no, k)
                    ...
                    checkpoint = torch.load(p_mlp_pre_train_checkpoint)
                    p_net_mlp.load_state_dict(checkpoint['p_net_state_dict'])
                    p_acc, p_conf = p_test(train_loader, p_net_mlp)
                    values[0][0] += p_acc
                    values[0][1] += p_conf
                    p_acc, p_conf = p_test(val_loader, p_net_mlp)
                    values[0][2] += p_acc
                    p_acc, p_conf = p_test(test_loader, p_net_mlp)
                    values[0][3] += p_acc
                    
                    
                    checkpoint = torch.load(p_linear_pre_train_checkpoint)
                    p_net_linear.load_state_dict(checkpoint['p_net_state_dict'])
                    p_acc, p_conf = p_test(train_loader, p_net_linear)
                    values[1][0] += p_acc
                    values[1][1] += p_conf
                    p_acc, p_conf = p_test(val_loader, p_net_linear)
                    values[1][2] += p_acc
                    p_acc, p_conf = p_test(test_loader, p_net_linear)
                    values[1][3] += p_acc
                    

                    checkpoint = torch.load(q_pre_train_checkpoint)
                    p_net_linear.load_state_dict(checkpoint['p_net_state_dict'])
                    p_acc, p_conf = p_test(train_loader, p_net_linear)
                    values[2][0] += p_acc
                    values[2][1] += p_conf
                    ...
                    s_net.p_net.copy(p_net_mlp)
                    q_acc = q_test(train_loader, s_net, tech)
                    values[3][0] += q_acc
                    values[3][1] += 0
                    q_acc = q_test(val_loader, s_net, tech)
                    values[3][2] += q_acc
                    q_acc = q_test(test_loader, s_net, tech)
                    values[3][3] += q_acc
                    ...
                    values[5][3] += q_acc
                values = values/k
                df = pd.DataFrame(values, columns = ['train acc','avg confidence','val acc','test acc'], index = ['P MLP Pretrain', 'P Linear Pretrain', 'P Linear Q Pretrain', 'Q Pretrain', 'P MLP Final', 'Q Final'])
                dataset_technique_path = os.path.join("results", filename, "SelectR_" + str(tech) + "_" + str(input_x))
                os.makedirs(os.path.dirname(os.path.join(dump_dir,dataset_technique_path)), exist_ok=True)
                df.to_csv(os.path.join(dump_dir,dataset_technique_path),float_format='%.3f')
