...
def select_loss_function(p, q, mask):
    #loss = (q*mask*torch.log(p+epsilon) - 0.5*q*mask*torch.log(q+epsilon)).sum(dim=1).mean()
...
def sample_loss_function(p, q, a, mask):
    #loss = torch.log((a*p).sum(dim=1) +epsilon) + (q*mask*torch.log(p+epsilon) - q*torch.log(q+epsilon)*mask).sum(dim=1)
    ...
    for tech in ["sample","select"]:
        for input_x in [False,True]:
            ...
            #for fold_no in [0]:  
                ...
                best_val = 0
                result_filename = "results/KL/09012020/"+filename+"/SelectR_"+str(tech)+"_"+str(input_x)+"/results/"+str(fold_no)+"_out.txt"
                result_log_filename = "results/KL/09012020/"+filename+"/SelectR_"+str(tech)+"_"+str(input_x)+"/logs/"+str(fold_no)+"_log.csv"
                model_filename = "results/KL/09012020/"+filename+"/SelectR_"+str(tech)+"_"+str(input_x)+"/models/"+str(fold_no)+"_best.pth"
                ...
                for epoch in range(1, n_epochs + 1):
                  ...
                  if((epoch%10==0) and (epoch>0)):
                      e_model_filename = "results/KL/09012020/"+filename+"/SelectR_"+str(tech)+"_"+str(input_x)+"/models/"+str(fold_no)+"_"+str(epoch)+".pth"
